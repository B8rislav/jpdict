...
describe('suggestionToItem', () => {
  ...
  it('passes through id / unit / text', () => {
    const item = suggestionToItem(option({ id: 'word-勉強', text: '勉強' }), t);
  ...
  it('word hint is the generic "parse as word" copy', () => {
    expect(suggestionToItem(option({ kind: 'word' }), t).hint).toBe('разобрать как слово');
  });

  it('verb hint appends the "verb" label to the gloss', () => {
    const item = suggestionToItem(option({ kind: 'verb', text: '勉強する', gloss: 'учиться' }), t);
  ...
  it('kanji hint omits the missing level', () => {
    const item = suggestionToItem(option({ kind: 'kanji', unit: 'kanji', gloss: 'усердие' }), t);
    expect(item.hint).toBe('усердие');
  });

  it('phrase hint is the "full sentence parse" copy', () => {
    const item = suggestionToItem(option({ kind: 'phrase', unit: 'phrase' }), t);
  ...
  it('reverse hint without a preview is the bare "by meaning" copy', () => {
    const item = suggestionToItem(option({ kind: 'reverse_word', text: 'study' }), t);
...
describe('historyToItem', () => {
  ...
  it('maps query_type to the badge unit', () => {
    expect(historyToItem({ id: '1', query: '勉', query_type: 'kanji' }, t).unit).toBe('kanji');
    expect(historyToItem({ id: '2', query: '私は', query_type: 'sentence' }, t).unit).toBe(
      'phrase',
    );
    expect(historyToItem({ id: '3', query: '勉強', query_type: 'word' }, t).unit).toBe('word');
  });

  it('defaults to the word unit when query_type is absent', () => {
    expect(historyToItem({ id: '4', query: '猫' }, t).unit).toBe('word');
  });

  it('uses the query as the visible text', () => {
    expect(historyToItem({ id: '5', query: '食べる', query_type: 'word' }, t).text).toBe('食べる');
  });
});
