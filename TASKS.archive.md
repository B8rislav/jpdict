# Tasks — Phase 1 (archived)

> **Archived 2026-06-30.** This is the Phase 1 record: the original build plan plus the
> redesign, refactor, docs, Anki, highlighting and SEO slices. Most of it is `[x]` done.
> Genuinely-open items (the Motion **A** section, R7's `logEffectFailures`, R9's optional
> husky hook) were carried forward to [TASKS.md](TASKS.md) Phase 2. The `P0–P4` blocks near
> the bottom are the *original* build plan — long since implemented, never checked off; kept
> here for history only. Active work lives in [TASKS.md](TASKS.md).
